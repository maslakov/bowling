...
public class Frame {
    ...
    public FrameResult toFrameResult(){
        return new FrameResult(Arrays.copyOf(attemptPinsCount,attemptPinsCount.length),score);
    }
    ...
    public boolean tryHit(int pinsKnockedDown){
        if (attemptIndex < attemptPinsCount.length) {
            ...
            if (state == FrameState.NOTCOUNTABLE){
                ...
                if (attemptPinsCount.length == MAX_FINAL_FRAME_CAPACITY){
                    if (state != FrameState.REGULAR){
                        attemptPinsCount[attemptIndex] = pinsKnockedDown;
                        attemptIndex++;
                        return true;
                    }else{
                        // cannot write 3rd item after regular two
                        return false;
                    }
                }
                return false;
